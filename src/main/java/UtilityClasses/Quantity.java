...
public class Quantity<U extends IMeasurable> {
    ...
    public Quantity<U> convertTo(U targetUnit) {
        ...
        // Temperature uses non-linear formulas: convert via base (Celsius)
        if (unit instanceof TemperatureUnit) {
            double celsius   = unit.convertToBaseUnit(value);        // → Celsius
            double converted = targetUnit.convertFromBaseUnit(celsius); // Celsius → target
            return new Quantity<>(converted, targetUnit);
        }

        // Linear units: standard factor-based conversion
        double base      = unit.convertToBaseUnit(value);
